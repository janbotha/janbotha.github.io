...
const images5 = ([
  ...
  {key: 2, image: "http://target.scene7.com/is/image/Target/GUEST_83705133-fb5e-4357-be37-94ae1392b915"},
  {key: 3, image: "http://target.scene7.com/is/image/Target/GUEST_83705133-fb5e-4357-be37-94ae1392b915"},
  {key: 4, image: "http://target.scene7.com/is/image/Target/GUEST_40a6190c-d9e3-4ddf-b06d-6db6a13927f3"}
]);

const { getByText, getByTestId } = render(<Carousel images={images5} />);

it('test carousel 5 image', () => {
  var img = document.createElement("img");
  img.alt="previous preview";
  img.className="primary-image not-selected-image";
  img.setAttribute('data-id', images5[4].key);
  img.setAttribute('data-testid', "imagePrev");
  img.src=images5[4].image;
  expect(getByTestId("imagePrev")).toEqual(img);

  img.alt="product preview";
  img.className="primary-image selected-image";
  img.setAttribute('data-id', images5[0].key);
  img.setAttribute('data-testid', "imageNow");
  img.src=images5[0].image;
  expect(getByTestId("imageNow")).toEqual(img);

  img.alt="next preview";
  img.className="primary-image not-selected-image";
  img.setAttribute('data-id', images5[1].key);
  img.setAttribute('data-testid', "imageNext");
  img.src=images5[1].image;
